...
def create_bar_chart(feedback):
    ...
    fig.update_layout(
        ...
        annotations=[
            ...
            dict(text=f'{correct_perc}%', x=0.5, y=0.82, font=dict(size=30, color='#f1f5f9'), showarrow=False),
            dict(text='Correct', x=0.5, y=0.68, font=dict(size=14, color='#94a3b8'), showarrow=False),
            ...
            # Main Title
            dict(text='Question_Breakdown', x=0.5, y=1, font_size=20, font_color="#f1f5f9", showarrow=False)
        ]
    )
    return fig
